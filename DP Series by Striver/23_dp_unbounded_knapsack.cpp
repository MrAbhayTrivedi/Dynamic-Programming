...
val=[5 11 13]
Output: 27
The theif can put items in the following way to maximize his stolen value:
2 items of wt 4 and 1 item of wt 2.
11+11+5=27
...
f(ind,W)-> Maximum value of items that the thief can steal from items with index 0 to ind capacity W of the knapsack.
...
final f(ind,W){
    ...
    int notTaken = 0 + f(ind-1,W);
    int taken = INT_MIN;
    if(wt[ind] <= W)
        taken = val[ind] + f(ind,W-wt[ind]);
...
// Function to solve the unbounded knapsack problem
int unboundedKnapsack_SO(int n, int W, vector<int>& val, vector<int>& wt) {
...
int main() {
    ...
    cout << "The Maximum value of items the thief can steal is " << unboundedKnapsack(n, W, val, wt) << endl;//Using Memoization
    cout << "The Maximum value of items the thief can steal is " << unboundedKnapsack_Tab(n, W, val, wt) << endl;//Using Tabulation
    cout << "The Maximum value of items the thief can steal is " << unboundedKnapsack_SO(n, W, val, wt) << endl;//Using Space Optimization
